...
if (!empty($_POST['user_check_eshop']) || !empty($_POST['report_eshop'])) {
...
if (!empty($_POST['report_eshop'])) {
//проверка рефера
cheak_ref($_SERVER['HTTP_REFERER']);

//проверка наличия заявки
$empty_dem = $db_pay_desk->empty_dem_eshop($did);
if (!empty($empty_dem)) {
	$errormsg = "Заявка оправлена на рассмотрение";
	$errortag = true;
	$db_pay_desk->demand_edit_nal('yn',$did,'demand_eshop');
	}
	else {
	$errormsg = "Данную заявку Вы можете оплатить в автоматическом режиме";
	$errortag = true;
	}
}
//вывод инфы по завке
if (!$error_did) {
$demand_info = $db_pay_desk->demand_info_eshop($did);

	if (!empty($demand_info)) {

$purse = $db_exchange->sel_purse_service($demand_info[0]['output']);
$us = $db_pay_desk->select_usluga($demand_info[0]['name_goods'],'reserv_goods');
$desc_pay = "Payment of goods : {$demand_info[0]['name_goods']}/{$demand_info[0]['pole1']}, ID:{$did}";
$des_cat = $db_pay_desk->select_cat($demand_info[0]['name_uslugi']);
$cur_out_info = $db_exchange->cur_out_info($demand_info[0]['output']);
$edit_out_val = trim(sprintf("%8.0f",$demand_info[0]['amount']));
...
if ($errortag || empty($_POST['user_check_eshop']) && empty($_GET['d'])){

echo "

<div style=\"display: none;\" id=\"d1\">
<br />
<form name=\"check_report\" id=\"check_report\" method=\"post\" action=\"http://wm-rb.net/user_check_eshop.aspx\" onSubmit=\"return Check_report()\">
...
<form name=\"check_demand\" id=\"check_demand\" method=\"post\" action=\"http://wm-rb.net/user_check_eshop.aspx\" onSubmit=\"return Check_demand()\">
					...
					<br />
                    <input type=\"submit\" name=\"user_check_eshop\" value=\"Проверить\" style=\"width:100px;
		    \"onmouseover=\"this.style.backgroundColor='#E8E8FF';\" onmouseout=\"this.style.backgroundColor='#f3f7ff';\"
		    id=\"cursor\">
                    </form><br /><br />";
}

if (!empty($_POST['user_check_eshop']) && !$errortag || !empty($_GET['d'])) {
include ("include/check_demand_eshop.aspx");
}
?>
	</td>
	</tr>
