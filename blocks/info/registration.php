    ...
    private function process() {
        ...
        switch ($P->action) {
            case "addpartner":
                ...
                sValidate::ComparePass($P->pass,$P->repass);
                ...
                if(!sValidate::$code) {
                    ...
                    if(!empty($dubl)) {
                        foreach($dubl as $d) {
                            if($d['email'] == $P->email) {$status = 1; $message = $this->lang['L_dubl_email'];}
                            if($d['host'] == $P->host) {$status = 1; $message = $this->lang['L_dubl_host'];}
                        }
                    }
                    ...
                    if(!$status) {
                        ...
                        $mail->to = $P->email;
                        $mail->subject = "[WM-RB.net] ".$this->lang['L_mail_reg'];
                        ...
                        $mail->smtpmail();
                        $status = 0; $message = $this->lang['L_reg_ok'];
                    }
                } else {
            ...
            case 'mess_pass';
                ...
                if(!sValidate::$code) {
                    $dubl = dataBase::DBmain()->select('partner',
                        ...
                        "where email='".$P->email."'");
                    if(!empty($dubl)) {
                        $pass = substr(sFormatData::wm_ReqID(),4,10);
                        dataBase::DBmain()->update('partner',array('password'=>md5($pass)),"where email='{$P->email}'");

                        $mail = mailSender::init();
                        $mail->to = $P->email;
                        $mail->subject = "[WM-RB.net] ".$this->lang['L_mail_repass'];
                        ...
                        $message = $this->lang['L_send_pass'];
                    } else {
                        $status = 1; $message = $this->lang['L_no_partner'];
                    }
            ...
            case 'enter';
                ...
                if(!sValidate::$code) {
                    ...
                    if(!empty($isset)) {
                        ...
                        $status = 1; $message = $this->lang['L_error_enter'];
                    }
                } else {
                    $status = 1; $message = sValidate::$message;
                }
