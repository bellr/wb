...
class gcBillPayment {
    ...
    public static function billWM() {

        $P = inputData::init();

        sValidate::isIntWidth($P->wmid,12,'L_bad_wmid');
        sValidate::isInt($P->pay_id);
        sValidate::isIntWidth($P->did,10,'L_bad_did');

        if(!sValidate::$code) {

            $r = Extension::Payments()->Webmoney()->x1((array)$P);
            ...
            Model::Demand()->update($P->did,$P->type_action,$value);
